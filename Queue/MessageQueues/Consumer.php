...
namespace Lof\MauticGdpr\Queue\MessageQueues;
...
class Consumer extends AbstractQueueProcessor
{
    ...
    public function processMessage(string $_data)
    {
        ...
        if ($_data) {
            ...
            if ($data && isset($data["mautic_contact_id"]) && $data["mautic_contact_id"]) {
                $this->mauticContact->deleteRecord((int)$data["mautic_contact_id"]);
            }
        }
    }
}
